...
if (!$_GET[no] || $_GET[no] < 0) $_GET[no]=0; //$_GET[no] 값이 안넘어 오거나 잘못된(음수)값이 넘어오는 경우 0으로 처리

#########################################################################

// 데이터베이스에서 페이지의 첫번째 글($_GET[no])부터 $page_size 만큼의 글을 가져온다.
// 검색어가 없을 때
...
$result_count=mysqli_query($conn,"select count(*) from $board " . $add_query);
...
$total_page = floor(($total_row - 1) / $page_size); #2003/11/2 (bug report by guy209)
...
$current_page = floor($_GET[no]/$page_size);

# $_GET[no] 을 통해서 페이지의 첫번째 글이 몇번째 글인지 전달된다.
# 따라서 페이지 사이즈로 나누면 현재가 몇번째 페이지인지 알수있다.
# $_GET[no] 이 10이고 페이지사이즈가 10 이라면 결과는 1이다. 앞서 페이지는 0부터 시작이라고 했으니 두번째 페이지임을 나타낸다.
# 그렇다면 $_GET[no] 이 11이라면 1.1 이 되어버린다. 11번째 글도 두번째 페이지에 존재하므로 0.1은 무의미하니 버린다.
# 그런데 $_GET[no] 이란 값이 $page_size 만큼씩 증가되는 값이기때문에 (0,10,20,30 과 같은 등차수열) 내림을 하는것 또한 무의미하다.
...
<tr>
	...
	<td height=20  bgcolor=white>&nbsp;
		<? if ($row[depth] > 0) echo "<img src=img/dot.gif height=1 width=" . $row[depth]*7 . ">└"?><a href=read.php?id=<?=$row[id]?>&no=<?=$_GET[no]?>&field=<?=$_GET[field]?>&search_word=<?=$_GET[search_word]?>><? $add="";
		if ($row[cmt_cnt]>0) $add = " [" . $row[cmt_cnt] . "]";
		echo strip_tags($row[title], '<b><i>') . $add;
		?></a>
	</td>
	<!-- 제목 끝 -->
	<!-- 이름 -->
	<td align=center height=20 bgcolor=white>
		<font  color=black>
			<a href="mailto:<?=strip_tags($row[email], '<b><i>');?>"><?=strip_tags($row[name], '<b><i>');?></a>
		</font>
	</td>
	<!-- 이름 끝 -->
	<!-- 날짜 -->
...
$page_num = $i+1; // 실제 페이지 값이 0부터 시작하므로 표시할때는 1을 더해준다. 페이지 0 -> 1
	...
	if ($_GET[no]!=$page){ //현재 페이지가 아닐 경우만 링크를 표시
...
<option value=name >글쓴이</option>   
    ...
    <td align=right><a href="write.php"> 글 쓰 기</a></td>
</form>
</tr>
<tr>
